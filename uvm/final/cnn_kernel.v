module cnn_kernel #(
    parameter KX = 2,  // Number of Kernel X
    parameter KY = 2,  // Number of Kernel Y
    ...
    generate
        for (
          ...
          assign  mul[mul_idx * M_BW +: M_BW]	=$signed({1'b0, i_in_fmap[mul_idx * I_F_BW +: I_F_BW]}) * $signed({1'b0, i_cnn_weight[mul_idx * W_BW +: W_BW]});
            always @(posedge clk or negedge reset_n) begin
                if (!reset_n) begin
        ...
        if(i_in_valid)begin
            for (k= 0; k < KY; k = k + 1) begin
                for (j= 0; j < KX; j = j + 1) begin
                  reg_r_mul[k][j] <= $signed(r_mul[(k*KX+j)*M_BW +: M_BW]);
                end
            end
        end
    end
    ...
    generate
        always @(*) begin
            ...
            for (acc_idx = 0; acc_idx < KY * KX; acc_idx = acc_idx + 1) begin
              acc_kernel[0 +: AK_BW] = $signed({1'b0, acc_kernel[0 +: AK_BW]}) + $signed({1'b0, r_mul[acc_idx*M_BW +: M_BW]});
            end
        end
    endgenerate
    always @(posedge clk or negedge reset_n) begin
   ...
   // integer k;
    ...
    always @(posedge clk) begin
        for (k= 0; k < KY; k = k + 1) begin
            for (j= 0; j < KX; j = j + 1) begin
              reg_weight[k][j] <= i_cnn_weight[(k*KX+j)*W_BW +: W_BW];
            end
        end
    end
    ...
    always @(posedge clk) begin
        if(i_in_valid)begin
            for (k= 0; k < KY; k = k + 1) begin
                for (j= 0; j < KX; j = j + 1) begin
                  reg_i_fmap[k][j] <= i_in_fmap[(k*KX+j)*I_F_BW +: I_F_BW];
                end
            end
        end
    end
