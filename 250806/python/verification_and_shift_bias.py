...
# conv1.weight
weight = model.conv1.weight.data.clone().cpu()
weight = weight.to(torch.int8)
print("\nconv1_weight.shape:", weight.shape)
with open(os.path.join(weight_bias_path, "conv1_weights.mem"), "w") as f:
    for out_ch in range(weight.shape[0]):
        for in_ch in range(weight.shape[1]):
            for i in range(weight.shape[2]):
                for j in range(weight.shape[3]):
                    val = weight[out_ch, in_ch, i, j].item()
                    hex_val = f"{(val & 0xFF):02x}"  # 8bit signed hex (2의 보수)
                    f.write(f"0x{hex_val}\n")
print("✅ 8bit signed conv1 weight 저장 완료: conv1_weight.mem")




# conv2.weight
weight = model.conv2.weight.data.clone().cpu()
weight = weight.to(torch.int8)
print("\nconv2_weight.shape:", weight.shape)
with open(os.path.join(weight_bias_path, "conv2_weights.mem"), "w") as f:
    for out_ch in range(weight.shape[0]):
        for in_ch in range(weight.shape[1]):
            for i in range(weight.shape[2]):
                for j in range(weight.shape[3]):
                    ...
                    hex_val = f"{(val & 0xFF):02x}"  # 8bit signed hex (2의 보수)
                    f.write(f"0x{hex_val}\n")
print("✅ 8bit signed conv2 weight 저장 완료: conv2_weight.mem")


# fc1.weight
weight = model.fc1.weight.data.clone().cpu()
weight = weight.to(torch.int8)
print("\nfc1_weight.shape:", weight.shape)
with open(os.path.join(weight_bias_path, "stage3_fc1_weights.mem"), "w") as f:
    for out_ch in range(weight.shape[0]):
        for in_ch in range(weight.shape[1]):
            val = weight[out_ch, in_ch].item()
            hex_val = f"{(val & 0xFF):02x}"  # 8bit signed hex (2의 보수)
            f.write(f"0x{hex_val}\n")
print("✅ 8bit signed fc1 weight 저장 완료: fc1_weight.mem")
